...
rm -r ../data/models/conditioned_bs_full_features
rm -r ../data/models/unconditioned_bs_full_features
rm -r ../data/models/descriptor_free_full_features
rm -r ../data/models/fingerprint_free_full_features
rm -r ../data/models/bs_free_full_features
...
# Train models:
python trainer.py\
    --name conditioned_bs_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    ...
    --gated\
    --with-seqid\
    > ../data/logs/conditioned_bs_full_features_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name unconditioned_bs_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    ...
    --gated\
    --with-seqid\
    > ../data/logs/unconditioned_bs_full_features_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name descriptor_free_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    ...
    --gated\
    --with-seqid\
    > ../data/logs/descriptor_free_full_features_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name fingerprint_free_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    ...
    --gated\
    --with-seqid\
    > ../data/logs/fingerprint_free_full_features_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name bs_free_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --db $BS_FREE_DB\
    --gated\
    --with-seqid\
    > ../data/logs/bs_free_seqid_full_features_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name aa_id_free_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --db $AA_ID_FREE_DB\
    --gated\
    --with-seqid\
    > ../data/logs/aa_id_free_full_features_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name protein_free_full_features\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --db $PROTEIN_FREE_DB\
