...
INFERENCES_CSV=../data/csv/inferences_no_seqid.csv
INFERENCES_CONDITIONED_CSV=../data/csv/inferences_conditioned_bs_no_seqid.csv

# clean run:
rm $INFERENCES_CSV
rm $INFERENCES_CONDITIONED_CSV
rm -r ../data/models/conditioned_bs_no_seqid
rm -r ../data/models/unconditioned_bs_no_seqid
rm -r ../data/models/descriptor_free_no_seqid
rm -r ../data/models/fingerprint_free_no_seqid
rm -r ../data/models/bs_free_no_seqid
rm -r ../data/models/aa_id_free_no_seqid
rm -r ../data/models/protein_free_no_seqid
rm -r ../data/models/molecule_free_no_seqid

# Train models:
python trainer.py\
    --name conditioned_bs_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $CONDITIONED_BS_DB\
    > ../data/logs/conditioned_bs_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name unconditioned_bs_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $UNCONDITIONED_BS_DB\
    > ../data/logs/unconditioned_bs_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name descriptor_free_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $DESCRIPTOR_FREE_DB\
    > ../data/logs/descriptor_free_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name fingerprint_free_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $FINGERPRINT_FREE_DB\
    > ../data/logs/fingerprint_free_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name bs_free_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $BS_FREE_DB\
    > ../data/logs/bs_free_seqid_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name aa_id_free_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $AA_ID_FREE_DB\
    > ../data/logs/aa_id_free_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name protein_free_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $PROTEIN_FREE_DB\
    > ../data/logs/protein_free_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &

python trainer.py\
    --name molecule_free_no_seqid\
    --folds $FOLDS\
    --runs $RUNS\
    --epochs $EPOCHS\
    --gated\
    --db $MOLECULE_FREE_DB\
    > ../data/logs/molecule_free_no_seqid_$(date +%Y%m%d_%H%M%S).log 2>&1 &
