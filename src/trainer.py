...
arg_parser.add_argument(
    "--only-as-esm",
    help="Active site ESM embedding used only, without whole-sequence.",
    default=False,
    action="store_true"
)
arg_parser.add_argument(
    "--only-enz-esm",
    help="Enzyme features represented only by the enzyme ESM, without AS annotation.",
    default=False,    
    action="store_true"
)
arg_parser.add_argument(
    "--only-as",
    help="Exclude AA identity from the features and use only active site information.",
    default=False,
    action="store_true"
)
...
for k, (train_indices, validation_indices) in enumerate(folds):
    ...
    if not a.with_esm:
        esm_hf = None
    print(f"Starting training fold {k+1}")
    ...
    train_data = KmClass(
        ...
        with_esm=a.with_esm, 
        only_as_esm=a.only_as_esm,
        esm_hf=esm_hf,
        only_enz_esm=a.only_enz_esm,
        only_as=a.only_as,
    )
    scalers = {
